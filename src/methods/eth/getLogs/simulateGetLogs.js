...
import blockNumToHash from "../../utils/blockNumToHash.js";
...
const tronTxInfoToEthLogs = (txInfo, txIndex) => {
  return txInfo.log.map((log, idx) => ({
    ...
    // blockHash: txInfo.
    blockNumber: numberToHex(txInfo.blockNumber),
    data: `0x${log.data}`,
...
/**
 * Simulates the eth_getLogs API by fetching all blocks + transaction
 * infos for blocks in the passed range. Slow!
 */
export default async function simulateGetLogs([filterObject], ctx) {
  ...
  const fromBlockNum = hexToNumber(fromBlock);
  const toBlockNum = hexToNumber(toBlock);
  // console.dir({ filterObject, fromBlockNum, toBlockNum }, { depth: null });
    ...
    await Promise.all(
      blockRange.map(async (blockNum) => {
        try {
          ...
          throw new Error(
            `error getting logs for block ${blockNum}: ${err.message}`
          );
        }
      })
    )
    ...
    .filter((l) => {
      ...
      for (let i = 0; i < topics.length; i++) {
        ...
        if (Array.isArray(topic)) {
          if (topic.includes(l.topics[i])) continue;
          return false;
        }
        if (l.topics[i] === topic) {
          continue;
