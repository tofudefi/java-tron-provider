...
```
*/
export const eth_getTransactionByHash = async ([txHash], ctx) => {
  // strip leading 0x
  ...
  return transaction.fromTron(tx, txInfo, blockHash);
};
...
*/
export const eth_getTransactionByBlockHashAndIndex = async (
  [_blockHash, _idx],
  ctx
) => {
  const idx = hexToNumber(_idx);
  // strip leading 0x
  const blockHash = _blockHash.substr(2);

  const body = { value: blockHash };
  const { data: block } = await ctx.tronClient.post(
    "/wallet/getblockbyid",
    body
  );

  if (!Object.keys(block).length) {
    throw new ClientError(`Unknown block ${blockHash}`);
  }
  /*
  console.dir(
  ...
  */
  ...
  return eth_getTransactionByHash([`0x${tx.txID}`], ctx);
};
...
export const eth_getTransactionByBlockNumberAndIndex = async (
  ...
  const idx = hexToNumber(_idx);
  // strip leading 0x
  const blockNum = hexToNumber(_blockNum);
...
*/
export const eth_getTransactionReceipt = async ([txHash], ctx) => {
  // strip leading 0x
  let hash = txHash.substr(2);
  const body = {
    value: hash,
  ...
  const blockHash = `0x${block.blockID}`;
  // console.log({ tx, txInfo, block });
  return transactionReceipt.fromTron(tx, txInfo, blockHash, blockIndex);
};

export const eth_pendingTransactions = async (_params, _ctx) => {
