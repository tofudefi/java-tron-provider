...
function extractFromAddress(parameter) {
    if (parameter.value) {
        ...
        if (v.owner_address) {
            return index_js_1.address.fromTronHex(v.owner_address);
        }
        if (v.transparent_from_address) {
            return index_js_1.address.fromTronHex(v.transparent_from_address);
        }
    }
    throw new Error(`could not extract "from" address ${JSON.stringify(parameter)}`);
}
function parseContract(tronTx) {
    ...
    const to = toAddress === null ? null : index_js_1.address.fromTronHex(toAddress);
    const value = index_js_1.numberToHex(parameter.value.amount || parameter.value.from_amount || 0);
...
exports.fromTron = (tronTx, tronTxInfo, blockHash) => {
    ...
    const blockNumber = index_js_1.numberToHex(tronTxInfo.blockNumber);
    // console.log(parameter.value);
    return {
        blockHash: blockHash,
        blockNumber: blockNumber,
        hash: `0x${tronTx.txID}`,
        gas: index_js_1.numberToHex(tronTxInfo.fee || 0),
        // TODO: dummy data, didnt find good Tron equivalents yet
        // gas: "0xc350", // 50000
        gasPrice: index_js_1.numberToHex(20000000000),
        // TODO: this should be the tx data of tron transaction (e.g. tronTx.data)
        input: "0x",
        nonce: index_js_1.numberToHex(21),
        v: "0x0",
