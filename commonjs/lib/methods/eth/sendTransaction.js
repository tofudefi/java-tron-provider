...
async function signAndBroadcast(tronTransaction, ctx) {
    ...
    const signedTronTransaction = tron_crypto_js_1.signTransaction(tronTransaction, privateKey);
    const { data } = await tronClient.post("wallet/broadcasttransaction", signedTronTransaction);
    // console.dir({ signedTronTransaction, data }, { depth: null });
    // handle known errors
    if (data.code) {
        const javaTronMessage = web3_1.default.utils.hexToAscii(`0x${data.message}`);
        const message = [
            data.code ? `${data.code}: ` : "",
            javaTronMessage,
...
*/
exports.eth_sendTransaction = async ([txInput = {}], ctx) => {
    if (!ctx.privateKey) {
    ...
    if (address.toLowerCase() !== from.toLowerCase()) {
        throw ethErrors.provider.unauthorized(`expected "from" param to be ${address} but received ${from}`);
    }
    if ("value" in txInput) {
...
async function sendTransferTransaction(txInput = {}, ctx) {
    ...
    const tronTx = await signAndBroadcast(await createTransaction({ from, to, amount }, ctx), ctx);
...
async function sendCreateSmartContractTransaction(txInput = {}, ctx) {
    ...
    if (!body.abi)
        delete body.abi;
    const { data: unsignedTronTx } = await ctx.tronClient.post(`/wallet/deploycontract`, body);
...
async function sendTriggerSmartContractTransaction(txInput = {}, ctx) {
    ...
    const { data: { transaction: unsignedTronTx }, } = await ctx.tronClient.post(`/wallet/triggersmartcontract`, body);
    // console.dir({ unsignedTronTx }, { depth: null });
    const tronTx = await signAndBroadcast(unsignedTronTx, ctx);
    // console.dir({ tronTx }, { depth: null });
    return `0x${tronTx.txid}`;
